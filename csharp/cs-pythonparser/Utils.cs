...
/*
    public class FileLineMap {
        ...
        public FileLineMap(string inputSource, uint line, uint column) {
            ...
            _content = MungeContent(encoding.GetString(rest), 8, false, false, true);
...
# "a = b ? c : d" -> a = (b ? c : d)
# "currentLookaheadToken || lastConsumedToken" -> currentLookaheadToken || LastConsumedToken
# "TokenType.STRICTFP" -> TokenType.STRICTFP
# "!(getToken(1).getImage().equals("yield")&&isInProduction("SwitchExpression"))" -> (!(GetToken(1).Image.Equals("yield") && IsInProduction("SwitchExpression")))
# "foo.toString()" -> foo.ToString()
# "lhs.isAssignableTo()" -> lhs.IsAssignableTo()
# "lhs.isMethodCall()||lhs.isConstructorInvocation()||lhs.isAllocation()" -> (lhs.IsMethodCall() || lhs.IsConstructorInvocation()) || lhs.IsAllocation()
# "isParserTolerant()||permissibleModifiers.contains(getToken(1).getType())" -> IsTolerant || permissibleModifiers.Contains(GetToken(1).Type)
# "currentLookaheadToken==null&&!((Expression)peekNode()).isAssignableTo()" -> (currentLookaheadToken == null) && (!((Expression) PeekNode()).IsAssignableTo())
# "currentLookaheadToken!=null||((Expression)peekNode()).isAssignableTo()" -> (currentLookaheadToken != null) || ((Expression) PeekNode()).IsAssignableTo()
# "getToken(1).getType()!=TokenType._DEFAULT" -> GetToken(1).Type != TokenType._DEFAULT
*/
}
